 ...
 */
public class CommitsLogCommand extends AbstractCommand<List<CommitInfo>> {

  private final String path;

  private boolean excludeDeletedCommits = true;

  private CommitsLogCommand(Builder builder) {
  ...
  public List<CommitInfo> execute() {
    try {
      ...
      if(commits.isEmpty()) {
        throw new RuntimeException(getNoCommitsErrorMessage());
      }
      return commits;
  ...
  private boolean hasDeletedCommit(RevCommit commit) {
    DiffCommand diffCommand = new DiffCommand.Builder(repository, commit.getName()).addPath(path).build();
  ...
  private String getNoCommitsErrorMessage() {
    String errorMessage = "There are no commits in this repository";
    if(Strings.isNullOrEmpty(path)) {
  ...
  public static class Builder extends AbstractCommand.Builder<Builder> {
    ...
    public CommitsLogCommand build() {
      return new CommitsLogCommand(this);
    }
  }
