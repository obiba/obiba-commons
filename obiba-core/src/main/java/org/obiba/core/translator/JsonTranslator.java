...
public class JsonTranslator implements Translator {
  ...
  public String translate(String key) {
    ...
    try {
      ...
      logger.warn("Translation of {} is not valid path: {}", key, e.getMessage());
      return key;
    } catch (ClassCastException e) {
      logger.warn("Translation of {} is not a string: {}", key, e.getMessage());
      return key;
    } catch (Exception e) {
      logger.warn("Translation of {} gives unknown error: {}", key, e.getMessage(), e);
      return key;
    }
  }
